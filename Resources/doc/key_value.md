...
`type` is set to `KeyValueRow::class` and `allow_add` and `allow_delete` are always `true`.


The KeyValueCollection
----------------------

To work with collections and the Symfony6 form layer, you can provide an adder
...
$builder->add('options', 'partitech_key_value', array(
    'required' => false,
...
`Partitech\AdvancedFormBundle\Form\KeyValueContainer` argument. A flexible
